...
//        guard type == .initial else { return }
        ...
        MoEngageCoreIntegrator.sharedInstance.enableSDKForPartner(workspaceId: appID, integrationType: .none)
        
        DispatchQueue.main.async {
            if let journifyAnonymousID = self.analytics?.anonymousId {
                MoEngageSDKAnalytics.sharedInstance.setUserAttribute(journifyAnonymousID, withAttributeName: MoEngageDestinationConstant.journifyAnonymousIDAttribute, forAppID: appID)
            }
        }
    ...
    public func identify(event: IdentifyEvent) -> IdentifyEvent? {
        ...
        if let journifyAnonymousID = self.analytics?.anonymousId {
            MoEngageSDKAnalytics.sharedInstance.setUserAttribute(journifyAnonymousID, withAttributeName: MoEngageDestinationConstant.journifyAnonymousIDAttribute, forAppID: apiKey)
        }
